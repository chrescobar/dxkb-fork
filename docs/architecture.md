...
- `src/components/views/` — generic `ResourceCollection`, `ResourceWorkspace`, controlled resource filtering, export formatting, and `EntityViewShell` composition. `EntityViewShell` reuses the organism landing navigation and content-frame primitives so resource views retain the Taxonomy layout across desktop and mobile. Profile-backed Genome, Feature, Epitope, Surveillance, Serology, Strain, and Domains and Motifs adapters support canonical collections and exact-scope member tabs; Taxonomy contributes immutable lineage scope for those collection tabs, while Feature retains the cross-core genome lineage query needed to include descendant taxa. Surveillance and Serology keep backend `id` as row identity and resolve public members by `sample_identifier` plus their optional discriminator (`pathogen_test_type` or `test_type`), presenting a choice when the sample alone is ambiguous. Strain and Domains and Motifs are list-only and use backend `id`; their selected-record details stay in the collection. `src/hooks/views/` owns TanStack Query collection/detail behavior, URL synchronization, and local explicit/all-matching selection state.
- Genome routes are implemented: `/genome` is the focused `genome` collection, and `/genome/[genomeId]` is the `genome_id` member. Features, Proteins, and Domains and Motifs use exact `genome_id` scopes; later-phase tabs remain capability-gated when their data/component contract is unavailable.
