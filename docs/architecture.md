...
- Genome routes are implemented: `/genome` is the focused `genome` collection, and `/genome/[genomeId]` is the `genome_id` member. The member route explicitly composes Overview, Genome Browser, Sequences, Features, Proteins, Protein Structures, Domains and Motifs, Experiments, and Interactions, capability-gating tabs whose data/component contract is not yet available.
