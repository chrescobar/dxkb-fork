...
- `src/components/views/` — generic `ResourceCollection`, `ResourceWorkspace`, controlled resource filtering, export formatting, and `EntityViewShell` composition. `EntityViewShell` reuses the organism landing navigation and content-frame primitives so resource views retain the Taxonomy layout across desktop and mobile. Profile-backed Genome and Feature adapters support canonical collections and exact-scope member tabs; Taxonomy contributes immutable lineage scope for Genome, while its Feature tab retains the cross-core genome lineage query needed to include descendant taxa. `src/hooks/views/` owns TanStack Query collection/detail behavior, URL synchronization, and local explicit/all-matching selection state.
