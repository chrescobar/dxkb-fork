...
const mocks = vi.hoisted(() => ({
  ...
  permanentRedirect: vi.fn((href: string) => { throw new Error(`NEXT_REDIRECT:${href}`); }),
}));
...
describe("Feature member route", () => {
  ...
  it("renders the grouped overview and metadata", async () => {
    render(await FeaturePage({ params: Promise.resolve({ featureId: feature.feature_id }), searchParams: Promise.resolve({}) }));
    expect(screen.getByRole("heading", { name: feature.patric_id })).toBeInTheDocument();
    ...
    expect(screen.getAllByRole("link", { name: "MERS-CoV" })[0]).toHaveAttribute("href", "/genome/1282460.2049");
    expect(await generateMetadata({ params: Promise.resolve({ featureId: feature.feature_id }), searchParams: Promise.resolve({}) })).toMatchObject({ title: `${feature.patric_id} | Feature` });
  });

  it("renders exact-feature interactions", async () => {
    render(await FeaturePage({ params: Promise.resolve({ featureId: feature.feature_id }), searchParams: Promise.resolve({ tab: "interactions" }) }));
  ...
  it("permanently redirects a PATRIC ID lookup to feature_id", async () => {
    ...
    await expect(FeaturePage({ params: Promise.resolve({ featureId: feature.patric_id }), searchParams: Promise.resolve({}) })).rejects.toThrow(`NEXT_REDIRECT:/feature/${feature.feature_id}`);
  });

  it("uses notFound for absent, inaccessible, and ambiguous records", async () => {
    mocks.getFeature.mockResolvedValue({ feature: null, usedAlternateId: false });
    await expect(FeaturePage({ params: Promise.resolve({ featureId: "missing" }), searchParams: Promise.resolve({}) })).rejects.toThrow("NEXT_NOT_FOUND");
    mocks.getFeature.mockRejectedValue(new DataApiError("Multiple records", 409, "ambiguous_member"));
    await expect(FeaturePage({ params: Promise.resolve({ featureId: "ambiguous" }), searchParams: Promise.resolve({}) })).rejects.toThrow("NEXT_NOT_FOUND");
  });

  it("preserves upstream failures", async () => {
    mocks.getFeature.mockRejectedValue(new Error("Feature backend unavailable"));
    await expect(FeaturePage({ params: Promise.resolve({ featureId: "PATRIC.1" }), searchParams: Promise.resolve({}) })).rejects.toThrow("Feature backend unavailable");
  });
});
