...
export function JobStatusPill() {
  ...
  const totalCount = Object.values(taskSummary).reduce(
    (sum, n) => sum + n,
    0,
  );

  const { data: jobsResult } = useJobsData({
    offset: 0,
    limit: 5,
  ...
  const statusGroups = [
    { count: completedCount, icon: CheckCircle2, className: "text-emerald-400" },
    { count: runningCount, icon: Loader2, className: "text-blue-300 animate-spin" },
    { count: queuedCount, icon: Clock, className: "text-white/60" },
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger
        render={
          ...
          >
            {statusGroups.map(({ count, icon: Icon, className }, i) => (
