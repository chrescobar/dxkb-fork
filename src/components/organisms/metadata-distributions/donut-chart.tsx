...
function chartData(data: DonutDatum[]): DonutDatum[] {
  ...
  return otherValue > 0 ? [...top, { label: "Others", value: otherValue }] : top;
}
  ...
  return (
    <Card className="relative rounded-lg">
      <CardHeader>
        ...
        <CardDescription>
          {total > 0 ? `${numberFormatter.format(total)} records in shown buckets` : "No facet data"}
        </CardDescription>
      </CardHeader>
      <CardContent>
        {slices.length === 0 ? (
          <p className="text-sm text-muted-foreground">No distribution data was returned.</p>
        ) : (
          <div className="grid items-center gap-4 md:grid-cols-[180px_1fr]">
            ...
            <ul className="flex flex-col gap-2">
              {slices.map((datum) => (
                <li key={datum.label} className="flex items-center justify-between gap-3 text-sm">
                  ...
                  <span className="font-medium">{numberFormatter.format(datum.value)}</span>
                </li>
              ))}
            </ul>
          </div>
