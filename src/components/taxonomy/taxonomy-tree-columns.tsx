...
export interface TreeTableMeta {
  ...
  onCheckboxClick: (row: Row<TaxonRecord>) => void;
  clearAnchor: () => void;
}
...
export const taxonomyColumns: ColumnDef<TaxonRecord>[] = [
  {
    id: "__select__",
    header: ({ table }) => (
      <input
        ...
        onChange={(event) => {
          (table.options.meta as TreeTableMeta).clearAnchor();
    ...
    cell: ({ row, table }) =>
      isPlaceholder(row.original) ? null : (
        <input
          ...
          onChange={() => {
            (table.options.meta as TreeTableMeta).onCheckboxClick(row);
          }}
          onClick={(event) => {
  ...
  },
  {
    ...
    cell: ({ row, table }) => {
      ...
      const loading = (
        table.options.meta as TreeTableMeta
      ).loadingParentIds.has(numericId(record));
      return (
        <span
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return typeof genomes === "number" ? numberFormatter.format(genomes) : "";
    },
    size: 100,
  },
  {
