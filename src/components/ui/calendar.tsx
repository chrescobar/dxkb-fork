      ...
      formatters={{
        formatMonthDropdown: (date) =>
          locale?.localize?.month(
            date.getMonth() as Parameters<
              NonNullable<Locale["localize"]>["month"]
            >[0],
            { width: "abbreviated" },
...
function CalendarDayButton({
  ...
  return (
    <Button
      variant="ghost"
      size="icon"
      data-day={`${String(day.date.getFullYear())}-${String(day.date.getMonth() + 1).padStart(2, "0")}-${String(day.date.getDate()).padStart(2, "0")}`}
